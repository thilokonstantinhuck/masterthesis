...
for i in range(compFirst, compLast + 1):
    ...
    train_X = data_coarse[~(data_coarse["Fish_ID"].isin(feedGroups[feedTest]))].iloc[:, first_wavelength_coarse:].values
    test_X = data_coarse[(data_coarse["Fish_ID"].isin(feedGroups[feedTest]))].iloc[:, first_wavelength_coarse:].values
    train_y = data_coarse[~(data_coarse["Fish_ID"].isin(feedGroups[feedTest]))][target].values
